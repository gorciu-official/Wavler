...
export class Lexer {
    ...
    private pushLineTerminator() {
        ...
        const last = this.tokens[last_index];
        const skipTypes = [TokenType.SEMICOLON, TokenType.LBRACE, TokenType.LPAREN, TokenType.COMMA, TokenType.COLON];

        if (!skipTypes.includes(last.type)) 
